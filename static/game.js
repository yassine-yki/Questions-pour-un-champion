...
// Adaptive question selection (solo): bring an unplayed question whose
// `difficulty` tag matches the current level into the upcoming slot, so the
// rest of showNextSoloQuestion's index logic stays unchanged. Falls back to
// adjacent difficulty, then leaves the original order if nothing is tagged.
...
function showNextSoloQuestion() {
    ...
    // Adaptive: choose the next question by the current difficulty level.
...
function handleMessage(msg) {
    switch (msg.event) {
        ...
        case 'players': 
            ...
            break;
        case 'gameStarting': 
            showMessage(msg.data.message || 'La partie commence !');
        ...
        case 'rematchStarted':
            showMessage(msg.data.message || 'Revanche !');
