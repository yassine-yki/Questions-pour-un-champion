...
(function() {
    ...
    // The close button called closeAuthCard(), which was never defined, so the
    // panel could never be dismissed. Define the full open/close/toggle set and
    // add click-outside + Escape dismissal.
    ...
    // Click outside the card (and outside the toggle pill) closes it.
    ...
    // without an account (you name yourself in the setup), so guest = close the
    // card and label the pill.
    ...
    // Quiz-type segmented control on the create screen (Classic / Wager).
    window.selectMultiQuizType = function(type) {
        ...
        const classic = document.getElementById('quizTypeClassic');
        const wager = document.getElementById('quizTypeWager');
        [classic, wager].forEach(el => el && el.classList.remove('is-on'));
        const target = type === 'wager' ? wager : classic;
        if (target) target.classList.add('is-on');
        const hint = document.getElementById('quizTypeHint');
        if (hint) {
            hint.textContent = (type === 'wager')
                ? 'Mode Mise : avant chaque question, chaque joueur mise une partie de ses points. Bonne réponse = gain de la mise, mauvaise = perte. Pas de buzzer.'
                : 'Questions classiques au buzzer.';
        }
    };
...
/* ============================================================
   WAGER MODE — host-selectable quiz type
   Buzzerless: before every question each active player stakes
   points, then everyone answers simultaneously. Correct = +stake,
   wrong/timeout = -stake. Implemented entirely client-side here by
   intercepting the wager-specific WS events in handleMessage.
   ============================================================ */
(function() {
    'use strict';
    ...
    function showWagerPhase(d) {
        ...
        const max = Math.max(0, mine);            // you may wager up to the points you hold
        ...
        const diffLabel = { 1: 'Facile', 2: 'Moyen', 3: 'Difficile' }[d.difficulty] || '';
        myWager = 0;
        ...
        o.innerHTML = `
            <div class="wager-card">
                <div class="wager-card__lbl">Manche ${d.round || 1}${diffLabel ? ' · ' + diffLabel : ''} · Question ${d.questionInRound || ''}/${d.questionsPerRound || ''}</div>
                <h2 class="wager-card__title">Placez votre <em>mise</em></h2>
                <p class="wager-card__sub">Vos points&nbsp;: <b>${bank}</b> · Bon&nbsp;: <b>+${base} et +${mult}×mise</b> · Faux&nbsp;: <b>−mise</b></p>
                ...
                <div class="wager-quick">
                    <button type="button" data-w="0">0</button>
                    <button type="button" data-w="half">½</button>
                    <button type="button" data-w="max">Tout (${max})</button>
                </div>
                <div class="wager-timer" id="wagerTimerLbl">${d.wagerTime || 15}s</div>
                <button class="btn btn--primary btn--lg" id="wagerConfirm" style="width:100%;">Valider la mise</button>
            </div>`;
    ...
    function onWagerAccepted(d) {
        ...
        if (card) {
            card.innerHTML = `
                <div class="wager-card__lbl">Mise placée</div>
                <h2 class="wager-card__title">Mise&nbsp;: <em>${d.amount}</em></h2>
                <p class="wager-card__sub">En attente des autres joueurs…</p>
    ...
    function showWagerQuestion(d) {
        ...
        if (ind) ind.textContent = `🎲 Votre mise : ${myWager}`;
    ...
    function onAnswerLocked(d) {
        ...
        if (ind && !/verrouillée/.test(ind.textContent)) ind.textContent += ' · réponse verrouillée';
    }
    ...
    function showWagerResult(d) {
        ...
        const rows = Object.entries(d.results || {}).map(([name, r]) => {
            ...
            return `<div class="wr-row ${cls}">
                        ...
                        <span class="wr-wager">mise ${r.wager}</span>
        ...
        o.innerHTML = `
            <div class="wager-card">
                <div class="wager-card__lbl">Réponse</div>
    ...
    // ── Language sync: the app UI is French, but selectedLanguage defaulted to
    // 'en' and was never loaded from storage, so dynamic text / server messages /
    // questions came back English. Load the saved language (default French),
    // reflect it on the settings toggle, and keep the toggle in sync on change.
    ...
    // Intercept the wager-specific events (and the buzzerless question) before
    // the original dispatcher, which doesn't know about them.
